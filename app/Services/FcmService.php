 ...
 *  - storage/app/firebase-service-account.json  (downloaded from Firebase Console)
...
class FcmService
{
    ...
    public function __construct()
    {
        $this->projectId  = config('services.firebase.project_id', '');
        $this->fcmUrl     = "https://fcm.googleapis.com/v1/projects/{$this->projectId}/messages:send";
        $cacert           = ini_get('curl.cainfo') ?: base_path('cacert.pem');
    ...
    /** Send to multiple tokens (batched per 500) */
    public function sendToTokens(
        array  $tokens,
        string $title,
        string $body,
        ...
        foreach (array_chunk($tokens, 500) as $chunk) {
            foreach ($chunk as $token) {
                $message          = $this->buildMessage($title, $body, $imageUrl, $data);
                $message['token'] = $token;
                $this->dispatch($message) ? $sent++ : $failed++;
            }
        }
    ...
    private function dispatch(array $message): bool
    {
        ...
        try {
            ...
            $response = Http::withOptions($this->sslOptions)->withToken($accessToken)
                ->post($this->fcmUrl, ['message' => $message]);
    ...
    private function getAccessToken(): string
    {
        ...
        if (!file_exists($keyFilePath)) {
            throw new \RuntimeException('Firebase service account JSON not found at: ' . $keyFilePath);
        }
        ...
        return $token['access_token'] ?? throw new \RuntimeException('Failed to obtain FCM access token.');
    }
}
